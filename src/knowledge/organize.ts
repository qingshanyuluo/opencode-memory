...
- 将具体业务事实上升为方法。例如不要记“某用户掉出某实验”，应记“如何判断某用户是对照组、未分桶还是受众变更；需要查哪些日志/配置/状态”。
...
- 跨项目可复用的平台手册或工具坑（数据库/日志/缓存/配置/部署/消息等通用平台领域），namespace 使用 personal；只适用于某仓库的实现仍使用项目 namespace。
